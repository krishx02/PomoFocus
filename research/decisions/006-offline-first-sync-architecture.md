...
- [ADR-013: BLE GATT Protocol Design](./013-ble-gatt-protocol-design.md) — BLE device outbox drain uses a dedicated Session Sync Service with chunked bulk transfer; phone receives sessions via BLE, then forwards to the API
- [ADR-016: BLE Client Libraries & Integration](./016-ble-client-libraries-integration.md) — react-native-ble-plx for mobile, Web Bluetooth for web; app-open sync trigger for v1 (no background BLE); shared sync orchestration in `packages/ble-protocol/`
