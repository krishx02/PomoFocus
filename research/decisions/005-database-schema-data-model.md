...
- [ADR-012: Security & Data Privacy](./012-security-data-privacy.md) — RLS + API gateway + JWT validation as defense-in-depth. Hard deletes align with GDPR right to erasure. Social visibility via scoped functions.
- [ADR-013: BLE GATT Protocol Design](./013-ble-gatt-protocol-design.md) — session records synced via BLE map to the `sessions` table; client-generated UUIDs enable idempotent inserts via `ON CONFLICT (id) DO NOTHING`
- [ADR-018: Social Features Architecture](./018-social-features-architecture.md) — social API endpoints use direct friendship JOINs for privacy enforcement. DB functions `is_friend_focusing()` and `did_friend_focus_today()` are repurposed as integration test helpers, not called in production.
- [ADR-019: Notification Strategy](./019-notification-strategy.md) — `devices` table stores `expo_push_token` column; `sessions` table drives goal nudge self-cancellation logic
